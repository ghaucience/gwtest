...
int cmd_request_nxp_pair(char *buff) {
  ...
  cmd_frame_send(CMD_REQUEST_NXP, NULL, 0);
  ...
  if (proto_frame_get_cmd((u8*)frame) != (CMD_REQUEST_NXP|0x8000) ) {
    return 2;
  }
  char *data = (char*)proto_frame_get_data((u8*)frame);
  ResLedErrorLed_t *res = (ResLedErrorLed_t*)data;
...
int cmd_request_st_amber(char *buff) {
  char frame[256];
  cmd_frame_send(CMD_REQUEST_ST_AMBER, NULL, 0);
  ...
  if (proto_frame_get_cmd((u8*)frame) != (CMD_REQUEST_ST_AMBER|0x8000))  {
    return 2;
  }
  char *data = (char*)proto_frame_get_data((u8*)frame);
  stResZWaveExclude_t *res =  (stResZWaveExclude_t*)data;
